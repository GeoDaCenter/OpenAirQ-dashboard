...
###### PM2.5 START #####
  ...
  output$nn_raster <- downloadHandler(
    filename = "Neural_Net.zip",
    content = function(file) {
      a <- tempdir()
      print(a)
      writeRaster(nn.raster, paste0(a, "/Neural_Net_Base"), format="raster", overwrite=TRUE)
      writeRaster(nn.spatial, paste0(a, "/Neural_Net_Spatial"), format="raster", overwrite=TRUE)
      writeRaster(nn.out, paste0(a, "/Neural_Net_Outlier"), format="raster", overwrite=TRUE)
      
      zip.path <- file.path(a, "Neural_Net.zip")
      shp.files <- list.files(a,
                              "Neural_Net", 
                              full.names = T)
      zip.cmd <- paste("zip -j",
